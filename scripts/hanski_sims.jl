...
ks = rodent_carrycap .* u"ha" # Predator carrycap
...
αs = @SMatrix [0.0 α12 α13 
               α21 0.0 α23 
               α31 α32 0.0]

# Init conditions
P = 0.0001 
Ns = rodent_carrycap .* u"ha"
tspan = 1:10000
P_timeline = Vector{typeof(P)}(undef, length(tspan))
Ns_timeline = Vector{typeof(Ns)}(undef, length(tspan))
model = (; P, Ns, ks, Ds, Es, ys, αs, cs, rs, d_high, v, e, P_timeline, Ns_timeline, t, tspan)

function pred_prey_sim(model)
    (; P, Ns, P_timeline, Ns_timeline, ks, tspan) = model
    P1 = P
    Ns1 = Ns
    @inbounds for i in tspan
        ...
        #     ks
        # end
        Ns2 = hanski_prey_timestep(P1, Ns1, ks, model)::typeof(Ns)
...
mm = MakieModel(model) do layout, m
    ...
    Makie.lines!(ax2, P_timeline; label=rodent_labels[1])
    linkxaxes!(ax1, ax2)
    axislegend(ax1)
    axislegend(ax2)
end
